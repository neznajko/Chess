...
namespace Casl {
    ...
    static constexpr offset_t KING_FROM[]
    {  
        25, // B
        95, // W
    };
...
public:
    ...
    static constexpr int PMOT_ROW[]{ 9, 2 }; // B W
    ...
    Unit* Travel
          ( const offset_t src, const offset_t dst );
    ...
    void Unregister
         ( Generator* const gen, const offset_t offset );
    void InLoopUnregister
         ( Generator* const gen, const offset_t offset );
    const Unit* GetUnit( const offset_t k ) const
    {
        return sqs[ k ].GetUnit();
    }
    color_t GetUnitColor( const offset_t k ) const;
    bool Check
         ( const offset_t k, const color_t color ) const {
